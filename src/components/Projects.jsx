            ...
            <div key={currentPage} className={`grid xl:grid-flow-col justify-items-center gap-6  ${direction === "left"? "slide-In-Left": "slide-In-Right"}`}>
              {
                currentProjects.map((item, i)=>{
                  ...
                  return (
                    <div onClick={()=>handleSelected(item.name)} key={i} className={`relative w-70 h-80 sm:w-115 sm:h-90 flex items-center justify-center rounded-md overflow-hidden shadow-[0_0_15px_rgba(255,255,255,0.3)] cursor-pointer ${item.name === selected? "shadow-[0_0_15px_rgba(255,255,255,0.3)]": ""} transition duration-300`}>
