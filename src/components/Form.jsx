...
export const Form = ()=>{
   ...
   return (
    ...
    <form id="contact" className="flex flex-col gap-4 items-center justify-center backdrop-blur-sm mt-8 p-5" onSubmit={handleSubmit(onSubmit)}>
        ...
        <input {...register("nombre",{required: "El nombre es obligatorio"})} className="w-full pl-10 text-zinc-200 bg-zinc-800 transition-all duration-300 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" placeholder="Nombre y Apellido" />
        <User className="absolute left-2 top-1/2 -translate-y-1/2 text-indigo-700" />
        </div>
        <div className="relative w-75 sm:w-160">
        <input type="email" {...register("email",{required: "El email es obligatorio", pattern: {value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/}})} className="w-full pl-10 text-zinc-200 bg-zinc-800 transition-all duration-300 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" placeholder="E-mail" />
        <Mail className="absolute left-2 top-1/2 -translate-y-1/2 text-indigo-700" />
        
        </div>
        <div className="relative w-75 sm:w-160">
        <input {...register("asunto",{required: "El asunto es obligatorio"})} className="w-full pl-10 text-zinc-200 bg-zinc-800 transition-all duration-300 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" placeholder="Asunto" />
        <Tag className="absolute left-2 top-1/2 -translate-y-1/2 text-indigo-700" />
        </div>
        <div className="relative w-75 sm:w-160">
        <textarea {...register("mensaje",{required: "El mensaje es obligatorio"})} className="resize-none text-zinc-200 transition-all duration-300 w-full pl-10 h-60 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" placeholder="Mensaje" />
