...
export const Form = ()=>{
   ...
   return (
    ...
    <form id="contact" className="flex flex-col gap-4 items-center justify-center backdrop-blur-sm mt-8 p-5" onSubmit={handleSubmit}>
        ...
        <input name="nombre" className="w-full pl-10 text-zinc-200 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" value={form.nombre} onChange={(e)=>handleChange(e)} placeholder="Nombre y Apellido" />
        <User className="absolute left-2 top-1/2 -translate-y-1/2 text-indigo-700" />
        </div>
        {errors.nombre && <p className="text-red-500 text-sm">{errors.nombre}</p>}
        <div className="relative w-75 sm:w-160">
        <input name="email" className="w-full pl-10 text-zinc-200 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" value={form.email} onChange={(e)=>handleChange(e)} placeholder="E-mail" />
        <Mail className="absolute left-2 top-1/2 -translate-y-1/2 text-indigo-700" />
        </div>
        {errors.email && <p className="text-red-500 text-sm">{errors.email}</p>}
        <div className="relative w-75 sm:w-160">
        <input name="asunto" className="w-full pl-10 text-zinc-200 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" value={form.asunto} onChange={(e)=>handleChange(e)} placeholder="Asunto" />
        ...
        <div className="relative w-75 sm:w-160">
        <textarea name="mensaje" className="resize-none text-zinc-200 w-full pl-10 h-60 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" value={form.mensaje} onChange={(e)=>handleChange(e)} placeholder="Mensaje" />
