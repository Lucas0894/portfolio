...
export const Form = ()=>{
   ...
   return (
    ...
    <form id="contact" className="flex flex-col gap-4 items-center justify-center backdrop-blur-sm mt-8 p-5" onSubmit={handleSubmit}>
        <input name="nombre" className="w-75 sm:w-160 text-zinc-200 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" value={form.nombre} onChange={(e)=>handleChange(e)} placeholder="Nombre y Apellido" />
        {errors.nombre && <p className="text-red-500 text-sm">{errors.nombre}</p>}
        <input name="email" className="w-75 sm:w-160 text-zinc-200 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" value={form.email} onChange={(e)=>handleChange(e)} placeholder="E-mail" />
        {errors.email && <p className="text-red-500 text-sm">{errors.email}</p>}
        <input name="asunto" className="w-75 sm:w-160 text-zinc-200 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" value={form.asunto} onChange={(e)=>handleChange(e)} placeholder="Asunto" />
        {errors.asunto && <p className="text-red-500 text-sm">{errors.asunto}</p>}
        <textarea name="mensaje" className="resize-none text-zinc-200 w-75 sm:w-160 h-60 bg-zinc-800 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-700" value={form.mensaje} onChange={(e)=>handleChange(e)} placeholder="Mensaje" />
